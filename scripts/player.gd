...
func _physics_process(delta):
	...
	sprite_2d.flip_v = gravity_dir < 0
...
func _on_block_builder():
	...
	$"../".add_child(_block)
	_block.position = Vector2(position.x + -bool_to_int(sprite_2d.flip_h) * 100, position.y)
	var dir_x = Input.get_axis("left", "right")
	var dir_y = Input.get_axis("up", "down")
	var dir = Vector2.RIGHT * -bool_to_int(sprite_2d.flip_h)
